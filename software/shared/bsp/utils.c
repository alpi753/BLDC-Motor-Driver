...
void gen_demo_telemetry(bldc_telemetry_t* telem_data)
{
    ...
    telem_data->rpm_actual = 1500.0f + 400.0f * bsp_sin_f(t) + 120.0f * bsp_sin_f(2.7f * t);
    telem_data->rpm_target = 1600.0f;

    {
        float i_amp = 0.8f + 0.3f * bsp_sin_f(0.8f * t);
        telem_data->current_phase_a = 2.0f + i_amp * bsp_sin_f(t * 1.5f);
        telem_data->current_phase_b = 2.0f + i_amp * bsp_sin_f(t * 1.5f + 2.094f);
        telem_data->current_phase_c = 2.0f + i_amp * bsp_sin_f(t * 1.5f + 4.188f);
    }

    {
        float v_amp = 0.5f + 0.1f * bsp_sin_f(0.3f * t);
        telem_data->voltage_phase_a = 24.0f + v_amp * bsp_sin_f(t * 0.3f);
        telem_data->voltage_phase_b = 24.0f + v_amp * bsp_sin_f(t * 0.3f + 2.094f);
        telem_data->voltage_phase_c = 24.0f + v_amp * bsp_sin_f(t * 0.3f + 4.188f);
    }

    telem_data->i_d = 0.3f * bsp_sin_f(0.7f * t);
    telem_data->i_q = 1.2f + 0.5f * bsp_sin_f(0.9f * t);
    ...
    telem_data->battery_current = 8.0f + 2.0f * bsp_sin_f(0.5f * t);
    ...
    telem_data->bemf_strength = (uint8_t)(fminf(255.0f, fabsf(0.01f * telem_data->rpm_actual * bsp_sin_f(1.5f * t))));
    telem_data->obs_confidence = (uint8_t)((0.7f + 0.3f * bsp_sin_f(0.4f * t)) * 100.0f);
    telem_data->pll_lock_status = (fabsf(bsp_sin_f(0.8f * t)) > 0.5f) ? 1U : 0U;
    telem_data->angle_error_deg = (uint8_t)(fabsf(5.0f * bsp_sin_f(1.1f * t)));
